                 ...
                 ?>	   
				   ...
				   <div id="block_bg" class="block">
                        <div class="navbar navbar-inner block-header">
                             <div class="muted pull-left"><i class="icon-check"></i>&nbsp;Liste Cable Alimentation(s)</div>
							  <div class="muted pull-right">
								Nombre cable Alimentation (s): <span class="badge badge-info"><?php  echo $count; ?></span>
							 </div>
                          </div>
			 ...
			 <?php	
			   ...
			   where NOT EXISTS 
	            ...
	            (select * from location_details where stdevice.id = location_details.id)
		         ...
		         OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		         and dev_status = 'NOUVEAU' and dev_name = 'central_processing_unit'
		         OR NOT EXISTS 
	            (select * from location_details where stdevice.id = location_details.id)
		         and dev_status = 'NOUVEAU' and dev_name = 'cpu'
		         OR NOT EXISTS 
...
<!-----------------------------------Content------------------------------------>
		...
		OR NOT EXISTS 
	    (select * from location_details where stdevice.id = location_details.id)
		and dev_status = 'NOUVEAU' and dev_name = 'cable alimenatation'
		OR NOT EXISTS 
	    (select * from location_details where stdevice.id = location_details.id)
		and dev_status = 'NOUVEAU' and dev_name = 'Cable_Alimentation'
		OR NOT EXISTS 
