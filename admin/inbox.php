...
$select = mysql_query("SELECT * FROM messages WHERE ID_RECEIVER=$session_id AND SENDER='respAdm' AND RECEIVER='$type_person' AND DELETED_FOR_MANAGER='no' ORDER BY TIME desc");
while($data=mysql_fetch_array($select)){
    ...
    echo '<td><a href="reply.php?id='.$donnees['respAdm_id'].'">Repondre</a></td>';
...
$select1 = mysql_query("SELECT  * FROM messages WHERE SENDER='fonctionnaire' AND RECEIVER='admin' ORDER BY TIME desc");

while($data1=mysql_fetch_array($select1)){
    $id_sender1 = $data1['ID_SENDER'];
    $sql1 = mysql_query("SELECT * FROM fonctionnaire where fonct_id=$id_sender1");
