  ...
  <div class="row-fluid">
      <div class="col-lg-12">
            <div class="alert alert-success alert-dismissable">
             ...
             <i class="icon-check"></i><strong> Imprimer tous le stock </strong>
            </div>
       </div>
    </div>
  </div>
...
"Times New Roman","serif"'>Tous le Stock<o:p></o:p></span></p>
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_status = 'NOUVEAU'")or die(mysql_error());
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_status = 'Utilise'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?>  
  <td width=171 valign=top style='width:128.35pt;border-top:none;border-left:
  none;border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_status = 'Repare'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?> 
...
<?php	
	           $count_item=mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_name = 'CPU' OR dev_name = 'central processing unit' OR dev_name = 'Unite Central'
			   OR dev_name = 'cpu' OR dev_name = 'c_p_u' OR dev_name = 'cpu'  
			   ORDER BY stdevice.id DESC");
	           $count = mysql_num_rows($count_item);
?>
 <tr style='mso-yfti-irow:4;mso-yfti-lastrow:yes'>
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_status = 'Endommage'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?> 
...
$new = mysql_query("select * from stdevice 
			   LEFT JOIN device_name ON stdevice.dev_id=device_name.dev_id
			   where dev_status = 'Dechet'")or die(mysql_error());
$count = mysql_num_rows($new);		                                               
?> 
  ...
  <p class=MsoNormal style='margin-bottom:0in;margin-bottom:.0001pt;line-height:
  normal'><span style='font-size:12.0pt;mso-bidi-font-size:11.0pt;font-family:
  "Times New Roman","serif"'>Dechets<o:p></o:p></span></p>
  </td>
  <td width=192 valign=top style='width:2.0in;border-top:none;border-left:none;
  border-bottom:solid windowtext 1.0pt;border-right:solid windowtext 1.0pt;
  mso-border-top-alt:solid windowtext .5pt;mso-border-left-alt:solid windowtext .5pt;
