...
<!-----------------------------------Content------------------------------------>
<?php
		$device_query = mysql_query("select stdevice.id as idd,dev_name,dev_desc,dev_brand ,dev_serial,dev_model,stlocation.stdev_id,stdevice.dev_status,stdev_location_name,firstname,lastname from stdevice,location_details,stlocation,device_name ,fonctionnaire WHERE stdevice.id = location_details.id AND location_details.stdev_id = stlocation.stdev_id AND stdevice.dev_id=device_name.dev_id AND stdevice.fonct_id=fonctionnaire.fonct_id AND dev_status = 'Endommage'  ORDER BY location_details.ld_id DESC")or die(mysql_error());
		...
		<td class="empty">
			...
			<td><a rel="tooltip" title="Repare" id="e<?php echo $id; ?>" href="#repair<?php echo $id; ?>" data-toggle="modal" class="btn btn-success"><div class="hide">Endommage</div>Repare</a></td>
			<?php }else{ ?>			
			<?php } ?>
	...
	<?php } ?>   
<?php 
		$device_query = mysql_query("select stdevice.id as idd,dev_name,dev_desc,dev_brand ,dev_serial,dev_model,stlocation.stdev_id,stdevice.dev_status,stdev_location_name,firstname,lastname from stdevice,location_details,stlocation,device_name ,resptech WHERE stdevice.id = location_details.id AND location_details.stdev_id = stlocation.stdev_id AND stdevice.dev_id=device_name.dev_id AND stdevice.respAdm_id=resptech.respAdm_id AND dev_status = 'Endommage'  ORDER BY location_details.ld_id DESC")or die(mysql_error());
		while($row = mysql_fetch_array($device_query)){
		$id = $row['idd'];
		$stdev_id = $row['stdev_id'];
		$dev_status = $row['dev_status'];
    ...
    <?php
		$device_query = mysql_query("select stdevice.id as idd,dev_name,dev_desc,dev_brand ,dev_serial,dev_model,stlocation.stdev_id,stdevice.dev_status,stdev_location_name,firstname,lastname from stdevice,location_details,stlocation,device_name ,admin WHERE stdevice.id = location_details.id AND location_details.stdev_id = stlocation.stdev_id AND stdevice.dev_id=device_name.dev_id AND stdevice.admin_id=admin.admin_id AND dev_status = 'Endommage'  ORDER BY location_details.ld_id DESC")or die(mysql_error());
		while($row = mysql_fetch_array($device_query)){
		$id = $row['idd'];
		$stdev_id = $row['stdev_id'];
		$dev_status = $row['dev_status'];
