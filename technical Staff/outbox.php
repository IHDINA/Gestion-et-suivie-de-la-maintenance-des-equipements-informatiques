...
$select = mysql_query("SELECT * FROM messages WHERE ID_SENDER=$session_id AND SENDER='respAdm' AND RECEIVER='fonctionnaire' AND DELETED_FOR_MANAGER='no' ORDER BY TIME desc");
...
$select = mysql_query("SELECT * FROM messages WHERE ID_SENDER=$session_id AND SENDER='respAdm' AND RECEIVER='admin' AND DELETED_FOR_MANAGER='no' ORDER BY TIME desc");
while($data=mysql_fetch_array($select)){
    $id_sender = $data['ID_RECEIVER'];
